...
perform_sim_tree = function(N, n, n.test=200, model='a',lambdas= matrix(c(5,5,
  ...
  accuracy.vals.CTree = matrix(0,N,n.lambdacombs) # Results from CTree
  accuracy.vals.CRF = matrix(0,N,n.lambdacombs) # Results from CRF
  accuracy.vals.rpart = rep(0,N) # Results for rpart
  accuracy.vals.rf = rep(0,N) # Results for random forest
  accuracy.vals.icp_rpart = rep(0,N) # Results for ICP+rpart
  accuracy.vals.icp_rf = rep(0,N) # Results for ICP+random forest
  accuracy.vals.anchor= rep(0,N) # Results for anchor regression
  accuracy.vals.anchor_boost = matrix(0,N,n.gamma) # Results from anchor boosting
  accuracy.vals.irf = matrix(0,N,n.lambda.irf) # Results from IRF
  accuracy.vals.train.CTree = matrix(0,N,n.lambdacombs) # Results from CTree, on training data
  accuracy.vals.train.CRF = matrix(0,N,n.lambdacombs) # Results from CRF, on training data
  accuracy.vals.train.rpart = rep(0,N) # Results for rpart, on training data
  accuracy.vals.train.rf = rep(0,N) # Results for random forest, on training data
  accuracy.vals.train.icp_rpart = rep(0,N) # Results for ICP+rpart, on training data
  accuracy.vals.train.icp_rf = rep(0,N) # Results for ICP+random forest, on training data
  accuracy.vals.train.anchor= rep(0,N) # Results for anchor regression, on training data
  accuracy.vals.train.anchor_boost = matrix(0,N,n.gamma) # Results from anchor boosting, on training data
  accuracy.vals.train.irf = matrix(0,N,n.lambda.irf) # Results from IRF, on training data

  auc.vals.CTree = matrix(0,N,n.lambdacombs) # Results from CTree
  auc.vals.CRF = matrix(0,N,n.lambdacombs) # Results from CRF
  auc.vals.rpart = rep(0,N) # Results for rpart
  auc.vals.rf = rep(0,N) # Results for random forest
  auc.vals.icp_rpart = rep(0,N) # Results for ICP+rpart
  auc.vals.icp_rf = rep(0,N) # Results for ICP+random forest
  auc.vals.anchor= rep(0,N) # Results for anchor regression
  auc.vals.anchor_boost = matrix(0,N,n.gamma) # Results from anchor boosting
  auc.vals.irf = matrix(0,N,n.lambda.irf) # Results from IRF
  auc.vals.train.CTree = matrix(0,N,n.lambdacombs) # Results from CTree, on training data
  auc.vals.train.CRF = matrix(0,N,n.lambdacombs) # Results from CRF, on training data
  auc.vals.train.rpart = rep(0,N) # Results for rpart, on training data
  auc.vals.train.rf = rep(0,N) # Results for random forest, on training data
  auc.vals.train.icp_rpart = rep(0,N) # Results for ICP+rpart, on training data
  auc.vals.train.icp_rf = rep(0,N) # Results for ICP+random forest, on training data
  auc.vals.train.anchor= rep(0,N) # Results for anchor regression, on training data
  auc.vals.train.anchor_boost = matrix(0,N,n.gamma) # Results from anchor boosting, on training data
  auc.vals.train.irf = matrix(0,N,n.lambda.irf) # Results from IRF, on training data
  
  rho.i.vals.train.CTree = matrix(0,N,n.lambdacombs) # Results from CTree, on training data
  rho.i.vals.train.CRF = matrix(0,N,n.lambdacombs) # Results from CRF, on training data
  rho.i.vals.train.rpart = rep(0,N) # Results for rpart, on training data
  rho.i.vals.train.rf = rep(0,N) # Results for random forest, on training data
  rho.i.vals.train.icp_rpart = rep(0,N) # Results for ICP+rpart, on training data
  rho.i.vals.train.icp_rf = rep(0,N) # Results for ICP+random forest, on training data
  rho.i.vals.train.anchor= rep(0,N) # Results for anchor regression, on training data
  rho.i.vals.train.anchor_boost = matrix(0,N,n.gamma) # Results from anchor boosting, on training data
  rho.i.vals.train.irf = matrix(0,N,n.lambda.irf) # Results from IRF, on training data
  rho.ii.vals.train.CTree = matrix(0,N,n.lambdacombs) # Results from CTree, on training data
  rho.ii.vals.train.CRF = matrix(0,N,n.lambdacombs) # Results from CRF, on training data
  rho.ii.vals.train.rpart = rep(0,N) # Results for rpart, on training data
  rho.ii.vals.train.rf = rep(0,N) # Results for random forest, on training data
  rho.ii.vals.train.icp_rpart = rep(0,N) # Results for ICP+rpart, on training data
  rho.ii.vals.train.icp_rf = rep(0,N) # Results for ICP+random forest, on training data
  rho.ii.vals.train.anchor= rep(0,N) # Results for anchor regression, on training data
  rho.ii.vals.train.anchor_boost = matrix(0,N,n.gamma) # Results from anchor boosting, on training data
  rho.ii.vals.train.irf = matrix(0,N,n.lambda.irf) # Results from IRF, on training data
  
  var_importance.CTree = array(0,c(N,p,n.lambdacombs))  # Results from CTree
  var_inclusion.CTree = array(0,c(N,p,n.lambdacombs))  # Results from CTree
  var_importance.CRF = array(0,c(N,p,n.lambdacombs)) # Results from CRF
  var_inclusion.CRF = array(0,c(N,p,n.lambdacombs)) # Results from CRF
  var_importance.rpart =  matrix(0,N,p)  # Results for rpart
  var_inclusion.rpart =  matrix(0,N,p)  # Results for rpart
  var_importance.rf =  matrix(0,N,p)  # Results for random forest
  var_inclusion.icp_rpart =  matrix(0,N,p)  # Results for ICP+rpart
  var_importance.icp_rpart =  matrix(0,N,p)  # Results for ICP+rpart
  var_importance.icp_rf =  matrix(0,N,p)  # Results for ICP+random forest
  var_importance.anchor=  matrix(0,N,p)  # Results for anchor regression
  var_importance.anchor_boost = array(0,c(N,p,n.gamma)) # Results from anchor boosting
  var_importance.irf = array(0,c(N,p,n.lambda.irf))# Results from IRF
  
  # Ensure reproducibility when using random methods
  seeds = sample(1:10000,size=N)
  for(i in 1:N){
    # Perform one simulation
    res.tmp = perform_sim_tree_oneiteration(n,n.test=n.test,model=model, lambdas_comb = lambdas, 
                                            gamma.anchor=gamma.anchor,lambda.irf = lambda.irf,
                                            min.leaf.size=min.leaf.size, CRF_sample_nodes=CRF_sample_nodes,
                                            eps.improvement=eps.improvement, n.thresh=n.thresh, n.cores=n.cores,num.trees=num.trees,
                                            include_CTree = include_CTree, include_CRF = include_CRF, 
                                            include_rpart = include_rpart, include_rf = include_rf ,
                                            include_icp_rpart = include_icp_rpart, include_icp_rf = include_icp_rf, 
                                            include_anchor = include_anchor, include_anchor_boost = include_anchor_boost, 
                                            include_irf = include_irf , prune=prune,normalise=normalise, 
                                            n_test_domains=n_test_domains, more.envirs=more.envirs, 
  ...
  res = list(accuracy.vals.CTree = accuracy.vals.CTree,
...
perform_sim_tree_oneiteration = function(n, n.test=200, model = 'a', lambdas_comb=NA,
  ...
  dat = generate_data_tree(n, model=model, more.envirs=more.envirs)
  df.use = as.data.frame(cbind(dat$y, dat$X))
  df.use.tree = as.data.frame(cbind(dat$y, dat$X, dat$z))
  names(df.use) = c('y', 'X1', 'X2', 'X3', 'X4', 'X5')
  names(df.use.tree) = c('y', 'X1', 'X2', 'X3', 'X4', 'X5', 'z')
  features = names(df.use)[-1]
  
  # Generate test data
  dat.test = generate_data_tree(n, model=model, new=T)
  ...
  names(df.test.use) = c('y', 'X1', 'X2', 'X3', 'X4', 'X5')
  names(df.test.use.tree) = c('y', 'X1', 'X2', 'X3', 'X4', 'X5', 'z')
  ...
  if (include_CRF) {
    r <- apply_CRF_core(df.use.tree, df.test.use.tree, features,
                             lambdas = lambdas_comb, n.thresh=n.thresh, eps.improvement =eps.improvement, 
                             min.leaf.size = min.leaf.size,n.trees=num.trees,
                             ...
                             verbose = FALSE, relax.constraint= relax.constraint, seed = 1)
  ...
  if (include_rf) {
    r <- apply_rf_core(df.use.tree.no.z, df.test.use.tree, features, min.leaf.size = min.leaf.size, num.trees=num.trees)
  ...
  if (include_icp_rf) {
    r <- apply_ICP_rf_core(df.use.tree, df.test.use.tree, features,  min.leaf.size = min.leaf.size, num.trees=num.trees)
  ...
  if (include_anchor_boost) {
    r <- apply_anchor_boost_core(df.use.tree, df.test.use.tree, features,python_module = "invariant_methods", lam=gamma.anchor)
