print_tree_sim_res_small = function(obj, lambdas_comb=c(0,0), gamma.anchor=0, lambda.irf=0,
  ...
  # Rule: bold if mean is max OR within 1 SE of the best row (best row's SE).
  # --------------------------------------------------------------------------
  bold_cmd = "\\textBF"
  eps = 1e-12
  ...
  if (include_CTree) for (i in 1:n.lambdacombs) {
    ...
    else add_stats(get_i(obj$accuracy.vals.CTree, i), get_i(obj$auc.vals.CTree, i))
  }
  if (include_CRF)   for (i in 1:n.lambdacombs) {
    ...
    else add_stats(get_i(obj$accuracy.vals.CRF,   i), get_i(obj$auc.vals.CRF,   i))
  }
  ...
  # --------------------------------------------------------------------------
  # Printing (TRAIN unbolded; TEST bolded by the wrapper above)
  # --------------------------------------------------------------------------
  if(include_CTree){
    for(i in 1:n.lambdacombs){
      ...
      if(! (setting=='invariant' & (lambda1.tmp!=lambda2.tmp | (lambda1.tmp ==0 & lambda2.tmp == 0 )))){
        ...
        print_performance_test(get_i(obj$accuracy.vals.CTree, i),
                               ...
                               sd=sd,
                               inclusion=get_i(obj$var_importance.CTree, i),
    ...
    for(i in 1:n.lambdacombs){
      ...
      if(!(setting=='invariant' & (lambda1.tmp!=lambda2.tmp | (lambda1.tmp ==0 & lambda2.tmp == 0 )))){
        ...
        print_performance_test(get_i(obj$accuracy.vals.CRF, i),
                               ...
                               sd=sd,
                               inclusion=get_i(obj$var_importance.CRF, i),
                               show_rho=multiple_test_domains,
                               suppress_rho=!multiple_test_domains)      
      }
    }
  ...
  if(include_rpart){
    ...
    print_performance_test(obj$accuracy.vals.rpart,
                           obj$auc.vals.rpart,
                           sd=sd,
                           inclusion=obj$var_importance.rpart,
                           show_rho=multiple_test_domains,
                           suppress_rho=!multiple_test_domains)
  }
  ...
  if(include_rf){
    ...
    print_performance_test(obj$accuracy.vals.rf,
                           obj$auc.vals.rf,
                           sd=sd,
                           inclusion=obj$var_importance.rf,
                           show_rho=multiple_test_domains,
                           suppress_rho=!multiple_test_domains)
  }
  ...
  if(include_icp_rpart){
    ...
    print_performance_test(obj$accuracy.vals.icp_rpart,
                           ...
                           sd=sd,
                           inclusion=obj$var_importance.icp_rpart,
                           show_rho=multiple_test_domains,
                           suppress_rho=!multiple_test_domains)
  }
  ...
  if(include_icp_rf){
    ...
    print_performance_test(obj$accuracy.vals.icp_rf,
                           obj$auc.vals.icp_rf,
                           sd=sd,
                           inclusion=obj$var_importance.icp_rf,
                           show_rho=multiple_test_domains,
                           suppress_rho=!multiple_test_domains)
  }
  ...
  if(include_anchor){
    ...
    print_performance_test(obj$accuracy.vals.anchor,
                           obj$auc.vals.anchor,
                           sd=sd,
                           inclusion=obj$var_importance.anchor,
                           show_rho=multiple_test_domains,
                           suppress_rho=!multiple_test_domains)
  }
  ...
  if(include_anchor_boost){
    for(i in 1:n.gamma.anchor){
      ...
      print_performance_test(get_i(obj$accuracy.vals.anchor_boost, i),
                             ...
                             sd=sd,
                             inclusion=get_i(obj$var_importance.anchor_boost, i),
                             show_rho=multiple_test_domains,
                             suppress_rho=!multiple_test_domains)
    }
  }
  ...
  if(include_irf){
    for(i in 1:n.lambda.irf){
      ...
      print_performance_test(get_i(obj$accuracy.vals.irf, i),
                             ...
                             sd=sd,
                             inclusion=get_i(obj$var_importance.irf, i),
                             show_rho=multiple_test_domains,
                             suppress_rho=!multiple_test_domains)
    }
  }
